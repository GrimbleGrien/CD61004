module general
  integer, parameter :: dp = kind(0.d0)
  integer :: Nmol, NAtom, NoMDStep, TotAtom, atom1, atom2
  real(kind=dp) :: TimeStep, Box, Sig, Eps, Rcut, Temp, Mass, EQMDStep
end module

module conversions
  use general, only: dp
  real(kind=dp), parameter :: TimeConv = 0.02418884326505d0    ! fs → au
  real(kind=dp), parameter :: EnerConv = 627.5094706d0         ! kcal/mol → au
  real(kind=dp), parameter :: LengthConv = 0.52917720859d0     ! Å → au
  real(kind=dp), parameter :: TempConv = 3.15773d5             ! K → au
  real(kind=dp), parameter :: MassConv = 9.10938291d-31        ! kg
  real(kind=dp), parameter :: Mp2Me = 1836.15267245d0          ! proton-to-electron ratio
end module


program mainprogram
  use general
  use conversions
  implicit none
  character(len=300) :: file1
  real(kind=dp), allocatable :: r(:,:), rm(:,:), v(:,:), Force(:,:)
  character(len=5), allocatable :: AtomLabel(:)
  real(kind=dp) :: PE, KE, t, t0, t1
  character(len=300) :: CoorFileName
  integer :: md_step, ierr

  call getarg(1, file1)
  open(unit=1, file=file1, action='read', iostat=ierr)
  if (ierr /= 0) then
    write(*,*) 'Problem opening input file. Use ./a.out md.input'
    stop
  endif

  read(1,*) Nmol
  read(1,*) NAtom
  read(1,*) TimeStep
  read(1,*) NoMDStep
  read(1,*) EQMDStep
  read(1,*) Temp
  read(1,*) CoorFileName
  read(1,*) Box
  read(1,*) Sig
  read(1,*) Eps
  read(1,*) Rcut
  read(1,*) Mass
  close(1)

  TotAtom = Nmol * NAtom  ! NMolecules * NAtomsperMolecule
  open(unit=5000, file='md.out', action='write')

  write(5000,"(a50,I8)")  "No. of Molecules = ", Nmol
  write(5000,"(a50,I8)")  "No. of Atoms per Molecule = ", NAtom
  write(5000,"(a50,F15.4)") "Time Step = ", TimeStep
  write(5000,"(a50,I8)") "No. of MD Steps = ", NoMDStep
  write(5000,"(a50,F15.4)") "Desired Temperature = ", Temp
  write(5000,"(a50,F15.4)") "Box Length = ", Box
  write(5000,"(a50,2F15.4)") "LJ Params: eps, sigma = ", Eps, Sig
  write(5000,"(a50,F15.4)") "Cutoff = ", Rcut
  write(5000,"(a50,F15.4)") "Mass = ", Mass
  write(5000,*)

  ! Unit conversions
  TimeStep = TimeStep / TimeConv
  Eps = Eps / EnerConv
  Sig = Sig / LengthConv
  Rcut = Rcut / LengthConv
  Box = Box / LengthConv
  Mass = Mass * Mp2Me
  Temp = Temp / TempConv

  allocate(r(TotAtom,3), rm(TotAtom,3), v(TotAtom,3))
  allocate(Force(TotAtom,3))
  allocate(AtomLabel(TotAtom))

  call initialize(TotAtom, CoorFileName, Temp, Mass, Box, r, v, AtomLabel)

  call cpu_time(t0)
  t = 0.d0; md_step = 0

  write(5000,"(a)") "Step           PE              KE              TE"

  do while (md_step < NoMDStep)
     call integrate(t, EQMDStep, TotAtom, Mass, Box, Temp, Rcut, Sig, Eps, AtomLabel, &
                    TimeStep, r, v, Force, KE, PE)
     md_step = md_step + 1
     write(5000,"(I10,2x,3F18.5)") md_step, PE*EnerConv, KE*EnerConv, (KE+PE)*EnerConv
     t = t + TimeStep
  enddo

  call cpu_time(t1)
  write(5000,"(a,2x,f10.2)") "Total MD runtime (sec): ", t1 - t0
  write(*,"(a,2x,f10.2)") "Total MD runtime (sec): ", t1 - t0

end program mainprogram
