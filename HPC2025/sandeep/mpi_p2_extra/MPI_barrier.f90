program q1
  ...
  msg=0 
! root process - recv and children processes - send 
  if(id/=0) then 
    call MPI_Send(id,1,MPI_Double_precision,0,tag,MPI_Comm_World,ierr)
  else 
    do i=1,p-1 
...
! root process - send and children processes - recv 
  if(id/=0) then 
      ...
      do i=1,p-1 
        call MPI_Send(id,1,MPI_Double_precision,i,tag,MPI_Comm_World,ierr)
      enddo
   endif
